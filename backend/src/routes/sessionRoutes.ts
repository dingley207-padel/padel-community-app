...
router.get(
  '/manager/sessions',
  authenticate,
  requireRole(['community_manager', 'super_admin']),
  getManagerSessionsHandler
);
...
// Get session bookings (community manager only)
router.get(
  '/:id/bookings',
  authenticate,
  requireRole(['community_manager', 'super_admin']),
...
// Create session (community manager only)
router.post(
  '/',
  authenticate,
  requireRole(['community_manager', 'super_admin']),
...
// Update session (community manager only)
router.put(
  '/:id',
  authenticate,
  requireRole(['community_manager', 'super_admin']),
...
// Cancel session (community manager only)
router.delete(
  '/:id',
  authenticate,
  requireRole(['community_manager', 'super_admin']),
